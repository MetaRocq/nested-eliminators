...
From MetaRocq.NestedElim.Formalization Require Import Lemma Positivity_condition Nested_to_mutual.
...
*)
  Definition pos_remove_rc_one acc arg :
    ...
    StrPosAcc nb_block up nup acc ->
    StrPosAcc nb_block up nup  (remove_rc_one acc arg).
  Proof.
  ...
  Definition pos_remove_rc (Γ : list argument) :
    ...
    StrPosAcc nb_block up nup (fold_left remove_rc_one Γ ( @nil argument, @nil argument, (fun n => n))).
  Proof.
    intros H.
    eapply spec_fold_All_check2 with (check := check_lax) (PX := PosArgBool _ _ _) (PAcc := StrPosAcc nb_block up nup ); cbn.
...
Section NestedToMutualInd.
  ...
  Definition pos_g_args_no_rc :
    All_telescope (fun Γ => positive_argument nb_g_block g_uparams_b g_nuparams
      (map check_lax Γ) false 0) (g_args_no_rc g_args).
  Proof.
  ...
  Definition g_args_no_rc_check_lax :
    All (fun arg => check_lax arg = false) (g_args_no_rc g_args).
  Proof.
    destruct (pos_remove_rc _ _ _ _ pos_g_args) as [[[lax_false pos_nargs] pos_rename] _].
    done.
  Qed.
  ...
  Definition g_args_no_rc_check_lax2 :
    map check_lax (g_args_no_rc g_args) = repeat false #|g_args_no_rc g_args|.
  Proof.
  ...
  Definition g_args_no_rc_false :
    Alli (fun i => positive_argument nb_g_block g_uparams_b g_nuparams
      (repeat false i) false 0) 0 (g_args_no_rc g_args).
  Proof.
  ...
  Definition length_g_args_no_rc :
    #|filter (fun t => ~~ check_lax t) g_args| = #|g_args_no_rc g_args|.
  Proof.
    unfold g_args_no_rc. rewrite -length_remove_rc; cbn. len; done.
  Qed.

  Notation nb_g_args_no_rc := (#|g_args_no_rc g_args|).
  ...
  Definition on_free_vars_rename_no_rc P i t :
    ...
    on_free_vars (shiftnP (nb_g_args_no_rc + i) P) (rename (rename_no_rc g_args) i t).
  Proof.
  ...
  Definition on_free_vars_argument_rename_no_rc P i arg k :
    ...
    on_free_vars_argument (shiftnP (nb_g_args_no_rc + i) P) k (rename_argument (rename_no_rc g_args) i arg).
  Proof.
    ...
    + destruct px as [K L], X2 as [U V]. split => //=; ofva_rnorc_aux.
      - eapply (Alli_mapi2 K U); cbn; intros.
        ...
        eapply on_free_vars_impl; tea; intros ?; rewrite !shiftnP_add; intros LL; apply_eq LL => //.
      - destruct x; cbn in *.
        change (argument_mapi (fun n0 : nat => PCUICSigmaCalculus.rename (shiftn n0 (rename_no_rc g_args)))) with (rename_argument (rename_no_rc g_args)).
  ...
  Definition pos_rename_no_rc i t :
    ...
    isup_notin g_uparams_b (nb_g_nuparams + nb_g_args_no_rc + i)
                                      (rename (rename_no_rc g_args) i t).
  Proof.
  ...
  Definition rename_rename_no_rc i t :
    rc_notin g_args i t -> rc_notin (g_args_no_rc g_args) i (rename (rename_no_rc g_args) i t).
  Proof.
  ...
  Definition rename_rename_arg_no_rc i arg :
    rc_notin_argument g_args i arg ->
    rc_notin_argument (g_args_no_rc g_args) i (rename_argument (rename_no_rc g_args) i arg).
  Proof.
  ...
  Definition pos_rename_no_rc_arg lax i arg :
    ...
    positive_argument nb_g_block g_uparams_b g_nuparams (map check_lax (g_args_no_rc g_args)) lax i
                                      (rename_argument (rename_no_rc g_args) i arg).
  Proof.
    unfold rc_notin_argument. unfold rc_notin_argument_bool.
    ...
    + clear Y. cbn in *. revert X1. generalize (List.rev (ind_uparams mdecl)).
      ...
      - change (argument_mapi (fun n : nat => PCUICSigmaCalculus.rename (shiftn n (rename_no_rc g_args))))
        with (rename_argument (rename_no_rc g_args)).
        destruct y.2.
        * apply h. done.
  ...
  Definition isup_notin_g_largs_no_rc :
    Alli (isup_notin g_uparams_b) (nb_g_nuparams + nb_g_args_no_rc) (g_largs_no_rc g_args g_largs).
  Proof.
  ...
  Definition wd_l_inst_uparams_no_rc :
    All (fun p =>
      ...
      * on_free_vars_argument xpredT (nb_g_largs + #|p.1|) p.2
      ) (l_inst_uparams_no_rc g_args g_largs inst_uparams_a).
  Proof.
  ...
  Definition size_l_inst_uparams_no_rc : #|l_inst_uparams_no_rc  g_args g_largs inst_uparams_a| = nb_l_uparams.
  Proof.
    len. rewrite -(List.length_rev (l_uparams_b)). eapply All2_length; tea.
  Qed.

  Definition fapp_l_inst_uparams_no_rc : All2 (fun n x => n = #|x.1|)
      (List.rev (uparams_nb_args l_uparams_b)) (l_inst_uparams_no_rc  g_args g_largs inst_uparams_a).
  Proof.
    rewrite -List.map_rev -(map_id (l_inst_uparams_no_rc  g_args g_largs inst_uparams_a)).
    eapply All2_map. eapply All_sym_inv.
    apply All2_map_left.
    eapply All2_impl; only 1 : apply spec_inst_uparams_a.
    intros [llargs arg] [cdecl pos]; cbn.
  ...
  Definition positive_true_all_no_rc :
    All (fun x => positive_argument nb_g_block g_uparams_b g_nuparams (map check_lax (g_args_no_rc g_args))
                      true (nb_g_largs + #|x.1|) x.2) (l_inst_uparams_no_rc  g_args g_largs inst_uparams_a).
  Proof.
    unfold l_inst_uparams_no_rc.
  ...
  Definition pos_inst_to_term :
    All2 (fun x y =>
        y.2 = false -> isup_notin g_uparams_b (nb_new_cxt_sub) (inst_to_term_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs x)
    ) (l_inst_uparams_no_rc g_args g_largs inst_uparams_a) (List.rev l_uparams_b).
  Proof.
    unfold l_inst_uparams_no_rc.
  ...
  *)
  Definition pos_inst_uparams_no_rc : forall n,
    isup_notinP l_uparams_b n ->
    isup_notin g_uparams_b nb_new_cxt_sub (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a n).
  Proof.
    intros n. unfold isup_notinP, andP, ind_notinP, sp_uparams_notinP.
    intros x%andb_prop; destruct x as [notInd notSpUparams].
  ...
  Definition inst_no_rc_preserve_isup_notin (k m : nat) t :
    m = nb_new_cxt_sub + k ->
    isup_notin l_uparams_b k t ->
    isup_notin g_uparams_b m t.[up k (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a)].
  Proof.
    intros ->; rewrite Nat.add_comm. apply on_free_vars_inst.
    intros n. rewrite <- shiftnP_add. apply on_free_vars_up.
  ...
  m = nb_new_cxt_sub + k ->
  All (isup_notin l_uparams_b k) l ->
  All (isup_notin g_uparams_b m) (map (fun t => t.[up k (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a)]) l).
  Proof.
    intros ->; intros H; apply All_map, (All_impl H).
  ...
  Alli (isup_notin l_uparams_b) k l ->
  Alli (isup_notin g_uparams_b) m
          (mapi_rec (fun i t => t.[up i (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a)]) l k).
  Proof.
    intros ->. intros X; eapply Alli_mapi_rec => //.
    intros. apply inst_no_rc_preserve_isup_notin => //.
  Qed.

  Definition wd_inst_uparams_no_rc : forall i, on_free_vars xpredT (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a i).
  Proof.
    intros i. unfold inst_uparams_no_rc.
    destruct (Nat.ltb_spec i #|(map (inst_to_term_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs) (l_inst_uparams_no_rc g_args g_largs inst_uparams_a))|).
  ...
  Definition pos_new_indices indices :
    ...
    positive_indices g_uparams_b g_nuparams (new_indices nb_g_block g_uparams_b g_nuparams g_args g_largs l_nuparams inst_uparams_a indices).
  Proof.
    ...
    unfold new_indices. rewrite !rev_app_distr ?map_app.
      ...
      - rewrite rev_involutive map_mapi -(mapi_map (fun i t => t.[up i (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a)])) map_app mapi_app.
  ...
  Definition cstr_extra_args : list argument :=
       g_args_no_rc g_args
    ++ map arg_is_free (g_largs_no_rc g_args g_largs)
    ++ map arg_is_free (mapi_rec (fun i t => t.[up i (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a)])
  ...
  Proof.
    ...
    - destruct (pos_remove_rc _ _ _ _ pos_g_args) as [[[lax_false pos_nargs] pos_rename] _].
      change ((fold_left remove_rc_one g_args ([], [], id)).1.2) with (g_args_no_rc g_args) in lax_false, pos_nargs.
  ...
  (** BEGIN ADDITION NO INDUCTIVE-INDUCTIVE **)
    Definition check_lax_specialize_argument Γ i:
      ...
      (map check_lax Γ) (map check_lax (mapi_rec (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a) Γ i)).
    Proof.
    ...
    Definition rc_notin_specialize_up Γ i nb_binders x n m :
      ...
      rc_notin_bool (map check_lax (cstr_extra_args ++ mapi_rec (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a) Γ nb_l_nuparams))
        n x.[up m (inst_uparams_no_rc nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a)].
    Proof.
      intros -> -> rc_notin_t.
    ...
    (~~ check_lax arg -> rc_notin_argument Γ nb_binders arg) ->
      ~~ (check_lax (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a (nb_l_nuparams + #|Γ| + nb_binders) arg)) ->
      rc_notin_argument
        (cstr_extra_args ++ (mapi_rec (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a) Γ nb_l_nuparams))
        nb_binders (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a (nb_l_nuparams + #|Γ| + nb_binders) arg).
    Proof.
      unfold negb, specialize_argument. intros pos_arg H lax_spe_arg.
      destruct pos_arg => //=.
      2: destruct (nth k (l_inst_uparams_no_rc g_args g_largs inst_uparams_a)) as [llargs []] eqn:Heqnth; cbn in * => //=.
    ...
    Definition rc_notin_argument_specialize_up lb nb_binders m arg :
      ...
      (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a m arg).
    Proof.
      ...
      + destruct (nth k (l_inst_uparams_no_rc g_args g_largs inst_uparams_a) err_arg) as [llarg arg_to_sub] eqn:H; cbn.
  ...
  Definition pos_specialize_argument Γ lax nb_binders arg :
    ...
    positive_argument nb_m_block g_uparams_b g_nuparams
      (map check_lax (cstr_extra_args ++ (mapi_rec (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a) Γ nb_l_nuparams)))
      lax nb_binders (specialize_argument nb_g_block g_uparams_b g_nuparams g_args g_largs inst_uparams_a (nb_l_nuparams + #|Γ| + nb_binders) arg).
  Proof.
    ...
    + rewrite <- Nat.add_assoc.
      destruct (nth k (l_inst_uparams_no_rc g_args g_largs inst_uparams_a) err_arg) as [llargs arg_to_sub] eqn:H.
      ...
      - eapply Alli_mapi.
        ...
        * cbn. intros n x isup_notin_x.
          ...
          by lia. done.
      - apply pos_arg_inc => //.
        assert (MX : positive_argument nb_g_block g_uparams_b g_nuparams (map check_lax (g_args_no_rc g_args)) lax (nb_g_largs + #|llargs|) arg_to_sub).
  ...
  Definition pos_specialize_ctor ctor :
    ...
    positive_constructor nb_m_block g_uparams_b g_nuparams (specialize_ctor nb_g_block g_uparams_b g_nuparams g_args g_largs l_nuparams inst_uparams_a ctor).
  Proof.
  ...
  Definition pos_specialize_idecl idecl :
    ...
    positive_one_inductive_body nb_m_block g_uparams_b g_nuparams (specialize_one_inductive_body nb_g_block g_uparams_b g_nuparams g_args g_largs l_nuparams inst_uparams_a idecl).
  Proof.
...
Section NestedToMutualIndb.
  ...
  Definition pos_nested_to_mutual_one_argument (acc : Acc) arg :
    ...
    (* res *)
    PosAcc (nested_to_mutual_one_argument nb_g_block g_uparams_b g_nuparams acc arg).
  Proof.
  ...
  Definition length1_nested_to_mutual_one_argument acc arg :
    #|(nested_to_mutual_one_argument  nb_g_block g_uparams_b g_nuparams acc arg).1| = 1 + #|acc.1|.
  Proof.
    destruct arg; cbn.
  ...
  Definition length_pos_left args acc :
    #|(fold_left (nested_to_mutual_one_argument nb_g_block g_uparams_b g_nuparams) args acc).1| = #|acc.1| + #|args|.
  Proof.
  ...
  Definition pos_nested_to_mutual_argument {args acc_indb}
    ...
    (* new_spec *)
    PosAcc (nested_to_mutual_argument nb_g_block g_uparams_b g_nuparams args acc_indb).
  Proof.
  ...
  Definition pos_nested_to_mutual_one_ctor ctor acc_indb
    ...
    (pos_acc_indb : All (positive_one_inductive_body (nb_g_block + #|acc_indb|) g_uparams_b g_nuparams) acc_indb):
    (* new_spec *)
    let x := nested_to_mutual_one_ctor nb_g_block g_uparams_b g_nuparams ctor acc_indb in
  ...
  Proof.
    ...
    cbn. set (x := nested_to_mutual_argument nb_g_block g_uparams_b g_nuparams ctor.(cstr_args) acc_indb) in *.
    repeat split; cbn => //.
    rewrite length_pos_left => //.
  Qed.

  Definition length2_nested_to_mutual_argument args new_indb :
    #|new_indb| <= #|(nested_to_mutual_argument nb_g_block g_uparams_b g_nuparams args new_indb).2|.
  Proof.
    unfold nested_to_mutual_argument.
  ...
  Definition pos_nested_to_mutual_ctors {ctors acc_indb}
    ...
    (pos_acc_indb : All (positive_one_inductive_body (nb_g_block + #|acc_indb|) g_uparams_b g_nuparams) acc_indb) :
    (* new_spec *)
    let x := nested_to_mutual_ctor nb_g_block g_uparams_b g_nuparams ctors acc_indb in
  ...
  Definition length2_nested_to_mutual_ctor ctors new_indb :
    #|new_indb| <= #|(nested_to_mutual_ctor nb_g_block g_uparams_b g_nuparams ctors new_indb).2|.
  Proof.
  ...
  Definition pos_nested_to_mutual_one_indb indb acc_indb
    ...
    (pos_acc_indb : All (positive_one_inductive_body (nb_g_block + #|acc_indb|) g_uparams_b g_nuparams) acc_indb):
    (* new_spec *)
    let x := nested_to_mutual_one_indb nb_g_block g_uparams_b g_nuparams indb acc_indb in
  ...
  Proof.
    ...
    cbn in *. set (x := nested_to_mutual_one_indb nb_g_block g_uparams_b g_nuparams indb acc_indb) in *.
    repeat split; cbn => //.
  Qed.
  ...
  Definition pos_nested_to_mutual_indb {indbs}
    ...
    * (All (positive_one_inductive_body (nb_g_block + #|x.2|) g_uparams_b g_nuparams) x.2))
    (nested_to_mutual_indb nb_g_block g_uparams_b g_nuparams indbs).
  Proof.
  ...
  Definition length_nested_to_mutual_indb indbs :
    #|(nested_to_mutual_indb nb_g_block g_uparams_b g_nuparams indbs).1| = #|indbs|.
  Proof.
